...
import { encryptWithMachineKey } from "@/lib/security/machineKey";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Encrypt config using machine-specific key
    const configJson = JSON.stringify(config, null, 2);
    const encryptedData = encryptWithMachineKey(configJson);
